...
$kegiatan = mysqli_escape_string($koneksi, $_POST['kegiatan']);
...
$nama_foto = strtolower($kegiatan);
$nama_foto = str_replace(' ', '-', $nama_foto) . '.' . $ekstensi;

$asal = $_FILES['foto']['tmp_name'];
$tujuan = '../../images/galery/';

if($_FILES['foto']['error'] == 0){

	if (file_exists($tujuan . $nama_foto)) unlink($tujuan . $nama_foto);

	$query = mysqli_query($koneksi, "INSERT INTO tbl_galery (foto,kegiatan) VALUES( '$nama_foto','$kegiatan')") or die(mysqli_error($koneksi));
